...
public abstract class ResourceReader implements ResourceIds {
    ...
    protected void initialize() {
        ...
        readClassIds(sysIdClass, "android", mIdNameToId);//读取系统资源文件,创建IDName-ID关系的缓存

        final String localClassName = getLocalClassName(mContext);//读取用户资源文件
        try {
        ...
        for (Map.Entry<String, Integer> idMapping : mIdNameToId.entrySet()) {
            mIdToIdName.put(idMapping.getValue(), idMapping.getKey());//ID-IDName 关系
        }
    }
