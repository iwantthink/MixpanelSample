...
public class MixpanelAPI {
    ...
    MixpanelAPI(Context context,
        ...
        this(context,
                referrerPreferences,
                token,
                //读取AndroidManifest.xml中的metaData信息
    ...
    MixpanelAPI(Context context, Future<SharedPreferences> referrerPreferences,
        ...
        //判断APP 版本是否有更新, 如果有 则发送APP_UPDATED事件
    ...
    public void connectEditor() {
        if (mUpdatesFromMixpanel != null && mUpdatesFromMixpanel instanceof ViewCrawler) {
            ViewCrawler crawler = (ViewCrawler) mUpdatesFromMixpanel;
            crawler.connectEditor();
        }
    }
    ...
    public static MixpanelAPI getInstance(Context context, String token, boolean optOutTrackingDefault) {
        ...
        synchronized (sInstanceMap) {
            ...
            // Context <---> MixpanelAPI 单例!!
            MixpanelAPI instance = instances.get(appContext);
            //为空,且拥有联网权限
            if (null == instance &&
                    ConfigurationChecker.checkBasicConfiguration(appContext)) {
            ...
            // 具体实现的功能待分析,表面是只是反射了bolts.AppLinks
    ...
    public void optOutTracking() {
        //清空 数据库中对应token 的Events 和 People数据
        getAnalyticsMessages().emptyTrackingQueues(
                new AnalyticsMessages.MixpanelDescription(mToken));
    ...
    /**
     * SDK-Version < 16   ||   禁用ViewCrawler  则返回 NoOpUpdatesFromMixpanel
     * 此外返回ViewCrawler
     *
     * @param context
     * @param token
     * @return
     */
    UpdatesFromMixpanel constructUpdatesFromMixpanel(final Context context,
                                                     final String token) {
        if (Build.VERSION.SDK_INT < MPConfig.UI_FEATURES_MIN_API) {
    ...
    /**
     * 注册一个 本地广播,监听[com.parse.bolts.measurement_event],收集`event_args`并上传
     *
     * @param context
     * @param mixpanel
     */
    ...
    /**
     * Token 对应 Map<Context,MixpanelAPI>
     */
    ...
    /**
     * 创建一个FutureTask 用来异步创建SharedPreferences
     */
